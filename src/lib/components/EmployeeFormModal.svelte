      ...
      <div class="mt-5 flex flex-col gap-4 border-t border-hairline pt-4">
        <div>
          ...
          <div class="flex items-center gap-2">
            ...
            <button type="button" class="sf-btn-secondary shrink-0" on:click={onResetPassword} disabled={!newPassword || $resetting.loading}>
        ...
        <div>
          ...
          <div class="flex items-center gap-2">
            <input type="file" accept="image/*" class="sf-input flex-1 !py-1.5 text-[12px]" on:change={(e) => (profileFile = e.currentTarget.files?.[0] ?? null)} />
            <button type="button" class="sf-btn-secondary shrink-0" on:click={onUploadProfile} disabled={!profileFile || $uploading.loading}>
